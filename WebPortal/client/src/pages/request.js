        ...
        {getuserdata.map((value, index)=>{
            return <div className="single_notification" key={index}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}> {value.request_type === 'company credentials' ?
                 (<>
                 <NavLink to ={`/uploadDocument/${value._id}`}><button onClick={() => { acceptReq(value._id,value.request_id) }}>Upload</button></NavLink>
                 <button onClick={() => { declineRequest(value._id,value.request_id) }}>Decline</button></>) 
                 : (<></>)}
                </div>
                <div style={{ display: 'flex', gap: '10px' }}>
                {value.request_type === 'user uploaded' ? (<><NavLink to ={`/showDocument/${value._id}/${value.request_id}`}><button> View </button></NavLink>
                <button onClick={() => { declineRequest(value._id,value.request_id) }}>Decline</button>
