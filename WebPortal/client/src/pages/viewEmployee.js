...
const ViewEmployee = () => {
 ...
 const fetchDoc = async(doc_url)=>{
  // let dummyUrl = "https://firebasestorage.googleapis.com/v0/b/dastakhat-86ccb.appspot.com/o/maya%40gmail.com%2FHabib%20University%2Fdummy.pdf?alt=media&token=01cbe009-1548-4272-899f-269edf6bd3f0"
  ...
  // console.log('link:: ',ref(storage, `${getuserdata.email_id}/Habib University/${doc_name}.pdf`))
  // getDownloadURL(ref(storage, `${getuserdata.email_id}/Habib University/${doc_name}.pdf`))
  // .then((url)=>{
  //   const xhr = new XMLHttpRequest();
  //   xhr.responseType='blob';
  //   xhr.onload=(event)=>{
  ...
  return (
    ...
    <div>
        ...
        <div className="employees_table part_two ">
          ...
          {getuserdata.attested_docs.map((element, id_) => {
          return (
            <tr key={id_}>
              <td>{id_+1}</td>
              ...
              <td>
                <NavLink  to ={`/showDoc/${element._id}/${emp_id}`}><button >view</button></NavLink> &nbsp;
                {/* to ={`/showDoc/${element.doc_status}`} */}
